...
contextBridge.exposeInMainWorld('kaida', {
  ...
  removeAllAccounts: (ids: string[]) => ipcRenderer.invoke('kaida:accounts:removeAll', ids),
 importExcel: () => ipcRenderer.invoke('kaida:accounts:importExcel'),
  ...
  startStudent: (id: string) => ipcRenderer.invoke('kaida:run:startStudent', id),
  stopAllStudents: () => ipcRenderer.invoke('kaida:run:stopAll'),
  applyStudentSettingsToAll: (id: string) => ipcRenderer.invoke('kaida:settings:applyAll', id),
