...
- 原用户加密文件在安装后立即复核时仍为原 SHA-256，账号页升级前后均显示未添加账号；但 2026-09-24 15:41 本机该文件后来从 307 字节改为 371 字节，修改原因尚未确认。不能据此证明原业务数据已成功解密或持续保留；此项仍为未验证。Windows 只有构建与清单验证，没有 Windows 真机。用户接受保留更新器 ZIP 缓存。
