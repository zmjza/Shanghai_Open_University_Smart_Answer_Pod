...
目标仓库：`zmjza/Shanghai_Open_University_Smart_Answer_Pod`；分支：`main`。当前工作仅在本机候选阶段，未创建版本提交或发布。

## 0.8.0 候选

- Tag / Release / 提交：未创建；GitHub 远端当前无版本 Tag 和 Release。
...
- 发布阻塞：本机只有自签名身份，没有正式 macOS Developer ID Application；签名构建报 `errSecInternalComponent`，候选验证包改以未签名模式生成。未进行 GitHub 推送、Release、在线 OTA、安装或重启验证。
